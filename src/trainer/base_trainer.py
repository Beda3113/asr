...
class BaseTrainer:
    ...
    def __init__(
        ...
        self.gradient_accumulation_steps = config.trainer.get("gradient_accumulation_steps", 1)
    ...
    def _train_epoch(self, epoch):
        ...
        # Initialize with default values in case no batches are processed
        last_train_metrics = {name: 0.0 for name in self.train_metrics.keys()}
        ...
        # If no batches were processed, use default values
        if last_train_metrics is None or len(last_train_metrics) == 0:
            last_train_metrics = {name: 0.0 for name in self.train_metrics.keys()}
