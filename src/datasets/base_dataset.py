...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        # 2. Apply waveform augmentations BEFORE spectrogram (CRITICAL!)
        audio = self.apply_wave_augs(audio)

        # 3. Encode text
        text_encoded = self.text_encoder.encode(text)

        # 4. Get spectrogram from (possibly augmented) audio
        ...
        # 5. Apply spectral augmentations (FreqMask, TimeMask) on spectrogram
    ...
    def get_spectrogram(self, audio):
        ...
        if self.instance_transforms is None:
            raise ValueError("instance_transforms is None, cannot compute spectrogram")
        if "get_spectrogram" not in self.instance_transforms:
            raise KeyError("'get_spectrogram' key not found in instance_transforms")
        return self.instance_transforms["get_spectrogram"](audio)

    def apply_wave_augs(self, audio):
        """
        Apply waveform augmentations before spectrogram computation.

        Args:
            audio (Tensor): audio waveform of shape (1, T)
        Returns:
            audio (Tensor): augmented audio waveform
        """
        if self.instance_transforms is None:
            return audio

        # Wave augmentations keys (applied BEFORE spectrogram)
        wave_augs_keys = ["gain", "add_noise", "pitch_shift"]

        for aug_name in wave_augs_keys:
            if aug_name in self.instance_transforms:
                aug = self.instance_transforms[aug_name]
                audio = aug(audio)
