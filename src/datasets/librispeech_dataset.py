...
class LibrispeechDataset(BaseDataset):
    def __init__(self, part, data_dir=None, *args, **kwargs):
        assert part in URL_LINKS or part in {"train_all", "train-clean-460"}
        ...
        if part == "train_all":
            parts = [p for p in URL_LINKS if "train" in p]
        elif part == "train-clean-460":
            parts = ["train-clean-100", "train-clean-360"]
        else:
            parts = [part]

        index = sum([self._get_or_load_index(p) for p in parts], [])
